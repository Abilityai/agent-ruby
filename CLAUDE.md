...
### Primary Access
- **[User] Personal Brand**: `$CONTENT_DIR/`
...
- Call Cornelius in headless mode: `cd $CORNELIUS_AGENT_DIR && claude -p "prompt" --output-format json`
...
- `/get-perspective <topic>` - Call Cornelius to get [User]'s unique perspective
...
4. **Direct requests**: Content created on-demand from [User]
...
- **LinkedIn**: `[User]_LinkedIn_Tone_of_Voice_Profile.md`
- **Twitter/X**: `[User]_Twitter_Tone_of_Voice_Profile.md`
- **Threads/Instagram**: `[User]_Text_Post_Tone_of_Voice_Profile.md`
...
**1. Get Perspective**
```bash
claude -p "What's [User]'s unique perspective on AI agent adoption barriers? Cite specific permanent notes." --output-format json
```

**2. Create Article**
```bash
claude -p "Create a LinkedIn article about the psychology of AI adoption using permanent notes. Apply [User]'s LinkedIn tone of voice." --output-format json
```
...
3. Format email with Substack frontmatter:
   ...
   [Newsletter content]
   ```
4. Send to Substack drafts email address (provided by [User])
5. Notify [User]: "Newsletter draft sent to Substack for review"

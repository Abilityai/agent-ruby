...
Call Cornelius in headless mode to extract Eugene's unique perspective on a topic from his knowledge base (1,883 notes, 550 permanent notes, 102 AI insights).
...
1. **Format Prompt for Cornelius**
   ...
   - Request Eugene's unique angle (not generic AI response)

2. **Call Cornelius (Headless Mode)**
   ```bash
   cd /Users/eugene/Dropbox/Agents/Cornelius
   claude -p "What's Eugene's unique perspective on [topic]? Cite specific permanent notes from the knowledge base. Focus on contrarian or non-obvious insights." --output-format json --timeout 60000
   ```
...
[Cornelius's response with Eugene's unique insights and perspectives]
...
✅ "What's Eugene's unique perspective on AI adoption barriers? Focus on psychological factors and cite permanent notes about dopamine, identity, or belief systems."

✅ "Find connections between Eugene's notes on Buddhism, neuroscience, and AI agent design. What non-obvious insights emerge?"

✅ "What contrarian views does Eugene have on AI replacing jobs? Look for insights that challenge conventional wisdom."
...
```
"What's Eugene's unique perspective on [TOPIC]? Provide:
...
```
"Find connections between [CONCEPT A] and [CONCEPT B] in Eugene's knowledge base. What non-obvious relationships exist? Cite specific permanent notes that bridge these concepts."
```

**For Contrarian Takes:**
```
"What's Eugene's contrarian perspective on [CONVENTIONAL WISDOM]? Look for permanent notes that challenge mainstream thinking. Explain the reasoning and evidence."
```
...
```bash
# Get perspective
response=$(cd /Users/eugene/Dropbox/Agents/Cornelius && \
  claude -p "Eugene's unique perspective on AI adoption barriers" \
  --output-format json)

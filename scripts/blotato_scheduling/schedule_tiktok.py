...
# Cloudinary URLs we just uploaded
cloudinary_urls = {
...
for idx, item in enumerate(tiktok_posts, 1):
    ...
    if filename not in cloudinary_urls:
        print(f"\n{idx}/{len(tiktok_posts)} - Skipping {filename} (no Cloudinary URL)")
        errors.append({'video': filename, 'error': 'No Cloudinary URL (file too large)'})
        continue

    video_url = cloudinary_urls[filename]
...
# Save results
with open('tiktok_scheduling_results.json', 'w') as f:
    json.dump({
        'scheduled': results,
        'errors': errors,
...
print("\n✓ Results saved to tiktok_scheduling_results.json")
